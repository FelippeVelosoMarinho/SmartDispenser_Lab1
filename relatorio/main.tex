...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        Botão confirmação     & Tátil, grande, GPIO 9 (BOOT)          & Push Button 12$\times$12mm \\
        Botões de volume      & GPIOs 6 (Vol+) e 7 (Vol-)             & Push Button 12$\times$12mm \\
...
\texttt{last\_confirmed\_slot}, \texttt{wifi\_rssi} e \texttt{uptime\_s}.
...
\begin{description}[leftmargin=4cm, style=nextline]
    \item[Pré-condição] Dispositivo ligado e conectado ao WiFi; agendamento configurado com modo silencioso ativado.
    \item[Passos]
        \begin{enumerate}
            \item Configurar agendamento com \texttt{silent\_mode: true} via frontend.
            \item Aguardar o horário agendado; o scheduler enfileira o comando com o campo \texttt{silent\_mode: true}.
            \item Verificar que o firmware, ao receber o comando via heartbeat, avança a roleta.
            \item Verificar que o motor de vibração entra em ciclo persistente (400~ms ligado / 250~ms desligado) enquanto \texttt{awaiting\_confirm} for \texttt{true}.
        ...
        \end{enumerate}
    \item[Resultado esperado] Remédios dispensados; alerta vibratório persistente ativado sem emissão de som; vibração cessa na confirmação.
    \item[Resultado obtido] Lógica de software verificada: campo \texttt{silent\_mode} propagado corretamente do frontend ao banco de dados, ao scheduler e ao comando entregue via heartbeat. Função \texttt{alertsTick()} implementada e integrada ao \texttt{loop()} do firmware. Comportamento físico do motor verificado parcialmente (vibração detectada, intensidade dependente da tensão de alimentação do circuito).
    \item[Status] \parcial
\end{description}
...
\begin{description}[leftmargin=4cm, style=nextline]
    \item[Pré-condição] Backend em execução; ESP32 conectado ao WiFi e enviando heartbeats; agendamento configurado para horário X via frontend.
    \item[Passos]
        \begin{enumerate}
            \item Configurar agendamento de dispensação para horário X via frontend.
            \item Aguardar o horário X.
            \item Verificar que o scheduler enfileira o comando na tabela \texttt{pending\_\allowbreak commands}.
    ...
    \item[Resultado obtido] Validado em integração completa em 08/06/2026: o scheduler detectou o horário dentro da janela configurada, enfileirou o comando \texttt{dispense} e o firmware executou a dispensação no heartbeat seguinte (latência máxima de 30~s). O servo avançou, o LED do período acendeu e o buzzer soou corretamente. Mecanismo de deduplicação (janela de 180~s) preveniu disparos duplos.
    \item[Status] \aprovado
\end{description}
...
\begin{description}[leftmargin=4cm, style=nextline]
    \item[Pré-condição] Firmware em execução; ESP32 conectado ao WiFi; agendamento configurado para horário X.
    \item[Passos]
        \begin{enumerate}
        ...
        \end{enumerate}
    \item[Resultado esperado] O alerta é ativado dentro da janela de até 30~s após o horário X, com período e modo corretos.
    \item[Resultado obtido] Validado em integração completa em 08/06/2026: o LED do período acendeu, o buzzer soou (modo normal) imediatamente após a entrega do comando via heartbeat, com latência de até 30~s em relação ao horário configurado, conforme esperado pela arquitetura de polling.
    \item[Status] \aprovado
\end{description}
...
\begin{description}[leftmargin=4cm, style=nextline]
    \item[Pré-condição] Cuidador com e-mail cadastrado no sistema; dispensação executada ou estoque crítico detectado.
    \item[Passos]
        \begin{enumerate}
            \item Verificar que o e-mail de notificação chega ao cuidador associado ao paciente nos eventos configurados.
            \item Verificar que notificações de estoque crítico são enviadas quando \texttt{critical\_stock} transiciona para \texttt{true}.
        \end{enumerate}
    \item[Resultado esperado] O cuidador vinculado recebe e-mail nas situações relevantes.
    \item[Resultado obtido] Notificações por e-mail implementadas via SMTP para os seguintes eventos: cadastro de cuidador (boas-vindas), transição para estoque crítico (\texttt{critical\_stock: true}) e falha de dispensação. Notificações push (Firebase Cloud Messaging) não foram implementadas.
    \item[Status] \parcial
\end{description}
...
\begin{description}[leftmargin=4cm, style=nextline]
    ...
    \item[Resultado obtido] Todos os três testes de conectividade da suíte pytest passaram: resposta HTTP~200, crescimento de uptime e RSSI dentro do intervalo válido.
    \item[Status] \aprovado
\end{description}
...
\begin{description}[leftmargin=4cm, style=nextline]
    ...
    \item[Passos]
        \begin{enumerate}
            \item Com volume em 0\%, pressionar BTN\_VOL\_UP (GPIO 6).
...
\begin{longtable}{p{6cm}cp{4cm}}
    ...
    Botões de volume (VOL\_UP/DOWN)     & Sim  & GPIOs 6 e 7; sem endpoint HTTP \\
