...
- [x] **Templates de E-mail (`app/services/templates.py`)**: Foram criados 5 templates HTML premium, responsivos e alinhados com o tom da marca:
  1. *Sucesso de Ingestão* (Verde)
  2. *Alerta de Dose Não Ingerida / Falha* (Vermelho)
  3. *Alerta de Estoque Crítico* (Laranja)
  4. *Alerta de Bateria Baixa* (Laranja/Vermelho)
  5. *Boas-vindas / Criação de Conta* (Teal - Novo!)
...
### E. Alerta de Bateria Baixa
* **Arquivo**: [iot.py](file:///home/felippe/Documentos/Github/UFMG/Lab1/SmartDispenser_Lab1/backend/app/api/endpoints/iot.py)
* **Gatilho**: Rota `POST /api/heartbeat` com `battery_level < 20.0`.
* **Fluxo**: O hardware reporta que o nível da bateria recarregável caiu abaixo de 20%. Dispara um e-mail de alerta para evitar que o dispenser fique offline.
* **Template HTML**: `get_low_battery_template`
